                          ...
                          <tbody>
                              ...
                              <tr>
                                  ...
                                  <td>
                                    ...
                                    class="form-inline"
                                    onsubmit="return confirm('Apakah pesanan kamu sudah Diterima?');" method="post">
                                    @csrf
                                
                                    <!-- TOMBOL VIEW ORDER KITA BUNGKUS JG DENGAN FORM AGAR RAPI -->
                                    <a href="{{ route('customer.view_order', $row->invoice) }}" class="btn btn-primary btn-sm mr-1">Lihat</a>
                                  
                                    <input type="hidden" name="order_id" value="{{ $row->id }}">
                                    @if ($row->status == 3)
                                        <button class="btn btn-secondary btn-sm">Terima</button>
